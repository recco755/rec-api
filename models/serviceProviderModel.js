...
module.exports = {
  ...
  likeBoostDelivery: async (req) => {
    ...
    const updated = await commonFunction.updateQuery(
      `UPDATE ${tableConfig.SERVICE_BOOST_DELIVERY} SET liked_at = NOW() WHERE id = ? AND consumer_user_id = ? AND dismissed_at IS NULL AND liked_at IS NULL`,
      [deliveryId, consumerId]
    );
    deferred.resolve({
      status: updated.affectedRows > 0 ? 1 : 0,
  ...
  /** Liked boosts still within offer window — Flash → Likes tab. */
  getHomeBoostLikesForConsumer: async (req) => {
    const deferred = q.defer();
    const consumerId = parseInt(req.body.user_id, 10);
    if (!consumerId) {
      deferred.resolve({ status: 0, message: "Invalid user_id", data: null });
